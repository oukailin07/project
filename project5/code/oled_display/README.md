# 地阔星 LCKFB-DKX 超声波测距 + 风扇控制终端

## 1. 接线总览

### 地阔星 LCKFB-DKX-STM32F103C8T6 排针引脚分配

> **板载资源说明**
> - 🔴 红色 LED D1 连接 **PA1**（串联 1kΩ，PA1 输出高时点亮）
> - 🟢 绿色 LED D2 连接 **PA2**（串联 1kΩ，PA2 输出高时点亮）
> - USB Type-C 接口：D- 接 **PA11**，D+ 接 **PA12**
> - SWD 调试口(CN1)：SWCLK-**PA14**, SWDIO-**PA13**, NRST, GND（4针排针）
> - BOOT0/BOOT1 独立跳线帽（不在主排针上）
> - ⚠️ PA2 同时用作 L298N ENA PWM → 风扇转动时绿色 LED 会随 PWM 亮度变化，属正常现象
...
```
   ...
   USB-C ════╣ ████████████████████████████████  ║
              ...
              ║  ┌─────────┐       ┌─────────┐   ║
           GND ║─┤ 1       ├───────┤ 1       ├─ 5V
    (板载LED)  ║─┤ 2  PC13 ├───────┤ 2   GND ├─ GND
              ║─┤ 3  PC14 ├───────┤ 3  3.3V ├─ 3V3
              ║─┤ 4  PC15 ├───────┤ 4   PB9 ├─ (空闲)
              ║─┤ 5   PA0 ├───────┤ 5   PB8 ├─ (空闲)
  🔴LED→──────║─┤ 6   PA1 ├───────┤ 6   PB7 ├────── OLED SDA
  🟢LED→ENA ──║─┤ 7   PA2 ├───────┤ 7   PB6 ├────── OLED SCL
              ║─┤ 8   PA3 ├───────┤ 8   PB5 ├─ (空闲)
              ║─┤ 9   PA4 ├───────┤ 9   PB4 ├────── HC-SR04 TRIG
              ║─┤10   PA5 ├───────┤10   PB3 ├────── HC-SR04 ECHO ⚡
              ║─┤11   PA6 ├───────┤11  PA15 ├─ (空闲)
              ║─┤12   PA7 ├───────┤12  PA12 ├─ (USB D+)
              ║─┤13   PB0 ├───────┤13  PA11 ├────── 按键1号脚(ROW1)
              ║─┤14   PB1 ├───────┤14  PA10 ├────── 按键2号脚(ROW2)
 L298N IN1 ────║─┤15  PB10 ├───────┤15   PA9 ├────── 按键3号脚(ROW3) 📡
 L298N IN2 ────║─┤16  PB11 ├───────┤16   PA8 ├────── 按键4号脚(ROW4)
 按键8(COL4)───║─┤17  PB12 ├───────┤17  NRST ├─ RST
 按键7(COL3)───║─┤18  PB13 ├───────┤18 3.3V  ├─ 3V3
 按键6(COL2)───║─┤19  PB14 ├───────┤19   GND ├─ GND
 按键5(COL1)───║─┤20  PB15 ├───────┤20   5V  ├─ 5V
              ║  └─────────┘       └─────────┘   ║
              ║          ↑ 排针序号从上到下        ║
              ╚══════════════════════════════════╝

  图例: ⚡=需禁用JTAG  📡=UART1烧录复用  🔴🟢=板载LED
```

> **⚠️ 排针序号说明**：上图为开发板俯视图（USB-C 朝上）。左排针(P2)和右排针(P1)各 20 脚，序号 1~20 从上到下依次排列。排针附近 PCB 丝印标注了对应的 GPIO 名称，接线时请以丝印为准。
...
| VCC | 5V | 供电 5V |
| GND | GND | 共地 |
| TRIG | PB4 | 触发信号，GPIO 推挽输出 |
| ECHO | PB3 | 回波信号，GPIO 浮空输入（需禁用 JTAG）|
...
| 键盘引脚 | STM32 | 说明 |
|----------|-------|------|
| 1 (ROW1) | PA11 | 行扫描，推挽输出 |
| 2 (ROW2) | PA10 | 行扫描，推挽输出 |
| 3 (ROW3) | PA9 | 行扫描，推挽输出 |
| 4 (ROW4) | PA8 | 行扫描，推挽输出 |
| 5 (COL1) | PB15 | 列检测，上拉输入 |
| 6 (COL2) | PB14 | 列检测，上拉输入 |
| 7 (COL3) | PB13 | 列检测，上拉输入 |
| 8 (COL4) | PB12 | 列检测，上拉输入 |
...
| L298N | STM32 | 说明 |
|-------|-------|------|
| ENA | PA2 | 电机使能/PWM 调速 (TIM2_CH3) |
| IN1 | PB10 | 电机方向控制 (5V 耐受) |
| IN2 | PB11 | 电机方向控制 (5V 耐受) |
| VCC | 12V/5V | 电机电源（根据风扇规格） |
| GND | GND | 共地 |

> **注意：** 仅使用单路电机输出驱动一个风扇。PB10/PB11 为 5V 耐受引脚，适配 L298N 的 5V 逻辑电平。
...
### 3.4 L298N 风扇驱动 — `l298n.h`

```c
void L298N_Init(void);               // 初始化 GPIO + TIM2 PWM

void L298N_Motor_Set(s8 speed);      // 风扇: speed = -100~100
void L298N_Motor_Stop(void);         // 停止风扇
```

#### 速度与方向对应关系

| speed 值 | 方向 | IN1(PB10) | IN2(PB11) | PWM 占空比 |
|----------|------|-----------|-----------|-----------|
| 100 | 正转（最快） | H | L | 100% |
| 50 | 正转（半速） | H | L | 50% |
| 0 | 停止 | L | L | 0% |
...
> **注意事项：**
> - PWM 频率 1kHz，分辨率 0.1%
> - L298N 模块需单独供电（12V/5V，根据风扇规格）
> - STM32 与 L298N 必须共地
> - PB10/PB11 为 FT (5V耐受) 引脚，适配 L298N 5V 逻辑电平
...
| 排针位置 | GPIO | 功能 | 所属模块 | 备注 |
|----------|------|------|----------|------|
| 左-7 | PA2 | ENA PWM | L298N (TIM2_CH3) | 🟢 板载绿色LED，PWM时闪烁 |
| 左-6 | PA1 | — | (未使用) | 🔴 板载红色LED，可作状态指示 |
| 左-15 | PB10 | IN1 方向 | L298N | FT 5V 耐受引脚 |
| 左-16 | PB11 | IN2 方向 | L298N | FT 5V 耐受引脚 |
| 左-17 | PB12 | COL4 | 矩阵键盘 (8号脚) | |
| 左-18 | PB13 | COL3 | 矩阵键盘 (7号脚) | |
| 左-19 | PB14 | COL2 | 矩阵键盘 (6号脚) | |
| 左-20 | PB15 | COL1 | 矩阵键盘 (5号脚) | |
| 右-13 | PA11 | ROW1 | 矩阵键盘 (1号脚) | USB D- 复用 |
| 右-14 | PA10 | ROW2 | 矩阵键盘 (2号脚) | UART1 RX 复用 |
| 右-15 | PA9 | ROW3 | 矩阵键盘 (3号脚) | UART1 TX 复用 |
| 右-16 | PA8 | ROW4 | 矩阵键盘 (4号脚) | |
| 右-7 | PB6 | SCL | OLED I2C | 软件模拟 I2C |
| 右-6 | PB7 | SDA | OLED I2C | 软件模拟 I2C |
| 右-9 | PB4 | TRIG 触发 | HC-SR04 | |
| 右-10 | PB3 | ECHO 回波 | HC-SR04 | ⚡需禁用 JTAG |
| CN1 | PA13 | SWDIO | 调试接口 | **必须保留，勿占用** |
| CN1 | PA14 | SWCLK | 调试接口 | **必须保留，勿占用** |

**剩余可用引脚：** PA0, PA3~PA7, PA15, PB0~PB1, PB5, PB8~PB9, PC13~PC15

> **⚠️ 注意事项：**
> - PA11 同时连接 USB D-，若 USB 连接电脑时，PA11 可能被拉低，影响按键扫描
> - PA9/PA10 为 UART1 烧录口，连接串口模块时会被占用
> - 接线时以板子**丝印标注**为准，不同批次排针顺序可能略有差异

...
static uint8_t  parse_type = 0;                 /* 当前包类型 */
...
void JY61P_Init(void)
{
    ...
    USART_DeInit(JY61P_USART);

    /* TX: PA9, 复用推挽输出 */
    GPIO_InitStructure.GPIO_Pin   = JY61P_TX_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AF_PP;
    GPIO_Init(JY61P_TX_PORT, &GPIO_InitStructure);

    /* RX: PA10, 浮空输入 */
    GPIO_InitStructure.GPIO_Pin  = JY61P_RX_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
    GPIO_Init(JY61P_RX_PORT, &GPIO_InitStructure);

    /* USART 配置: 波特率可配, 8N1 */
    USART_InitStructure.USART_BaudRate            = JY61P_BAUDRATE;
    USART_InitStructure.USART_WordLength          = USART_WordLength_8b;
    USART_InitStructure.USART_StopBits            = USART_StopBits_1;
    USART_InitStructure.USART_Parity              = USART_Parity_No;
    USART_InitStructure.USART_HardwareFlowControl = USART_HardwareFlowControl_None;
    USART_InitStructure.USART_Mode                = USART_Mode_Rx | USART_Mode_Tx;
    USART_Init(JY61P_USART, &USART_InitStructure);

    /* 使能 RXNE 中断 */
    USART_ITConfig(JY61P_USART, USART_IT_RXNE, ENABLE);
...
{
    if (USART_GetITStatus(JY61P_USART, USART_IT_RXNE) != RESET)
    {
        uint8_t byte = (uint8_t)USART_ReceiveData(JY61P_USART);

        /* 清除溢出标志 */
        if (USART_GetFlagStatus(JY61P_USART, USART_FLAG_ORE) != RESET)
        {
            (void)USART_ReceiveData(JY61P_USART);
        }
